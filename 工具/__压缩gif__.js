    ...
    return new Promise((r, j) => {
        ...
        if (!reg.test(filePath)) {
            ...
            sharp(filePath, { animated: true, limitInputPixels: false })
                .resize(400)
                .gif()
                .toFile(filePath.replace(".gif", "_400.gif"), (err, info) => {
                    if (err) {
                        console.log("err", err)
                        return
                    }
                    console.log("处理完成-> 下一个")
                    // 取出gif的第一帧，命名为 _gif.png
                    // exec(`ffmpeg -i ${filePath} -ss 1 -f image2 ${filePath.replace(".gif", "_gif.png")}`, (err, stdout, stderr) => {
                    //     if (err) {
                    //         console.log("err", err)
                    //         return
                    //     }
